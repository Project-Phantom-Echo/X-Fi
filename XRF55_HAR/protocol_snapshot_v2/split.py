...
* unseen_scenes_two_rooms and unseen_scenes_train_*: every unordered pair of
  scenes trains, and the complementary pair is tested with a frozen recipe.
...
PROTOCOL_NAMES = (
    ...
    "unseen_scenes_train_1_3",
    "unseen_scenes_train_1_4",
    "unseen_scenes_train_2_3",
    "unseen_scenes_train_2_4",
    "unseen_scenes_train_3_4",
    "unseen_scene_unseen_subject",
)

# Existing names retain their membership. New names carry the catalog/rotation ID
# through training, tuning, manifests, output directories and run provenance.
LEGACY_PROTOCOL_NAMES = PROTOCOL_NAMES
NUMBERED_SUBJECT_PROTOCOLS = tuple(f"unseen_subjects_{i:03d}" for i in range(1, 217))
ROOM_SUBJECT_PROTOCOLS = tuple(f"unseen_room_subject_rotation_{i}" for i in range(1, 4))
PROTOCOL_NAMES += NUMBERED_SUBJECT_PROTOCOLS + ROOM_SUBJECT_PROTOCOLS


@lru_cache(maxsize=1)
def subject_split_catalog():
    """Frozen numbered assignments matching the HTML explorer."""
    data = json.loads(Path(__file__).with_name("subject_splits.json").read_text())
    if data["source_scenes"] != [2, 3, 4] or len(data["splits"]) != 216:
        raise ValueError("Unexpected subject split catalog")
    return tuple(tuple(tuple(pair) for pair in row) for row in data["splits"])


def new_protocol(name: str) -> Protocol:
    if name in NUMBERED_SUBJECT_PROTOCOLS:
        row = subject_split_catalog()[NUMBERED_SUBJECT_PROTOCOLS.index(name)]
        validation = frozenset(pair[0] for pair in row)
        test = frozenset(pair[1] for pair in row)
        train = frozenset(SUBJECT_IDS) - validation - test
        selectors = (Selector(ALL_SCENES, subjects=train),
                     Selector(ALL_SCENES, subjects=validation),
                     Selector(ALL_SCENES, subjects=test))
        description = (
            f"Unseen-subject split {NUMBERED_SUBJECT_PROTOCOLS.index(name) + 1}: "
            f"validation IDs {sorted(validation)}, test IDs {sorted(test)}. "
            "Draws from Scenes 2–4; matching Scene-1 IDs follow their partition."
        )
    elif name in ROOM_SUBJECT_PROTOCOLS:
        train_scene, val_scene, test_scene = ((2, 3, 4), (3, 4, 2), (4, 2, 3))[
            ROOM_SUBJECT_PROTOCOLS.index(name)]
        held_out = SCENE_SUBJECTS[f"Scene{val_scene}"] | SCENE_SUBJECTS[f"Scene{test_scene}"]
        selectors = (Selector(("Scene1", f"Scene{train_scene}"), subjects=frozenset(SUBJECT_IDS) - held_out),
                     Selector((f"Scene{val_scene}",)), Selector((f"Scene{test_scene}",)))
        description = (
            f"Train Scenes 1+{train_scene}; validate Scene {val_scene}; test Scene {test_scene}. "
            "Exclude evaluation subjects from Scene-1 training; retain P30."
        )
    else:
        raise ValueError(f"unknown new protocol: {name}")
    def count(selector):
        return 1100 * sum(len(SCENE_SUBJECTS[scene] & selector.subjects)
                          if selector.subjects is not None else len(SCENE_SUBJECTS[scene])
                          for scene in selector.scenes)
    return Protocol(name=name, description=description, train=selectors[0],
                    evaluations={"validation": selectors[1], "test": selectors[2]},
                    validation=frozenset({"validation"}),
                    expected=(count(selectors[0]), {"validation": count(selectors[1]), "test": count(selectors[2])}))
...
def build_protocol(name: str) -> Protocol:
    ...
    three people each.
    """
    if name in NUMBERED_SUBJECT_PROTOCOLS or name in ROOM_SUBJECT_PROTOCOLS:
        return new_protocol(name)
    if name == "trial_split":
        return Protocol(
            name=name,
            description=(
    ...
    scene_pair_protocols = {
        "unseen_scenes_train_1_3": (("Scene1", "Scene3"), ("Scene2", "Scene4")),
        "unseen_scenes_train_1_4": (("Scene1", "Scene4"), ("Scene2", "Scene3")),
        "unseen_scenes_train_2_3": (("Scene2", "Scene3"), ("Scene1", "Scene4")),
        "unseen_scenes_train_2_4": (("Scene2", "Scene4"), ("Scene1", "Scene3")),
        "unseen_scenes_train_3_4": (("Scene3", "Scene4"), ("Scene1", "Scene2")),
    }
    if name in scene_pair_protocols:
        train_scenes, test_scenes = scene_pair_protocols[name]
        train_samples = sum(33000 if scene == "Scene1" else 3300
                            for scene in train_scenes)
        test_samples = {
            scene: 33000 if scene == "Scene1" else 3300 for scene in test_scenes
        }
        return Protocol(
            name=name,
            description=(
                f"Scenes {train_scenes[0][-1]} and {train_scenes[1][-1]} train; "
                f"Scenes {test_scenes[0][-1]} and {test_scenes[1][-1]} test. "
                "All released samples and all 20 trials are retained. This has "
                "no validation split and must use a configuration frozen elsewhere."
            ),
            train=Selector(train_scenes, repetitions=ALL_TRIALS),
            evaluations={scene: Selector((scene,)) for scene in test_scenes},
            expected=(train_samples, test_samples),
        )
