...
Evaluate validation after every epoch without opening test trials. Grid cells
are ranked using their fixed final-epoch validation score over seeds 1 and 2.
"""
...
def main() -> None:
    ...
    validation_loader = DataLoader(
        validation_data,
        generator=torch.Generator().manual_seed(args.seed + 1000000),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Validation must not advance the training random-number streams.
        ...
        try:
            with torch.random.fork_rng():
                validation_loss, validation_accuracy, validated_samples = run_epoch(
                    model, validation_loader, device, optimizer=None,
        ...
        epoch_history.append({
            "epoch": epoch, "train_loss": final_train_loss,
            "train_accuracy": final_train_accuracy,
            "validation_loss": validation_loss,
            "validation_accuracy": validation_accuracy,
            "train_samples": trained_samples, "validation_samples": validated_samples,
        })
        history_path = args.output_dir / "history.json"
        temporary = history_path.with_suffix(".tmp")
        temporary.write_text(json.dumps(epoch_history, indent=2) + "\n")
        temporary.replace(history_path)
        print(f"epoch={epoch:03d} validation_loss={validation_loss:.6f} "
              f"validation_accuracy={validation_accuracy:.6f}", flush=True)
